...
_OFFSET_SEQUENCE: str = rf"(?:{_OFFSET}(?:[ \t]+{_OFFSET})*)"

# XXX: OWORD TBYTE MMWORD and so on.
_WIDTH: str = r"(?:byte|word|dword|qword|xmmword|ymmword|zmmword)"

_PTR: str = r"(?:ptr)"

_MEMORY_OPERAND_MODIFIER: str = rf"(?:(?:{_OFFSET_SEQUENCE}[ \t]+)?(?:{_WIDTH}(?:[ \t]+{_PTR})?)?(?:[ \t]+{_OFFSET_SEQUENCE})?)"
...
# XXX: This doesn't enforce matching brackets, which is impossible with re
_ONE_OR_MORE_OPEN_BRACKETS: str = r"(?:\[(?:[ \t]*\[)*)"
_ONE_OR_MORE_CLOSE_BRACKETS: str = r"(?:\](?:[ \t]*\])*)"

_EFFECTIVE_ADDRESS_WITH_SEGMENT_REGISTER: str = rf"(?:(?:{_ONE_OR_MORE_OPEN_BRACKETS}[ \t]*)?{_SEGMENT_REGISTER}[ \t]*:[ \t]*{_CONSTANT_EXPRESSION}(?:[ \t]*{_ONE_OR_MORE_CLOSE_BRACKETS})?)"

# TODO: All the other forms of memory operands
_EFFECTIVE_ADDRESS: str = rf"(?:{_EFFECTIVE_ADDRESS_WITH_SEGMENT_REGISTER})"

# XXX: This will allow `offset qword ptrfs:0x10`
_MEMORY_OPERAND: str = rf"(?:(?:{_MEMORY_OPERAND_MODIFIER}[ \t]*)?{_EFFECTIVE_ADDRESS})"

_OPERAND: str = rf"(?:{_IMMEDIATE}|{_REGISTER}|{_LABEL}|{_MEMORY_OPERAND})"

_INSTRUCTION_PREFIX: str = "(?:rep(?:n?[ez])?|lock|notrack|cs|data16|addr32)"

# XXX: This allows any number of instruction prefixes
_INSTRUCTION_STATEMENT: str = rf"(?:(?:{_INSTRUCTION_PREFIX}[ \t]+)*{_MNEMONIC}(?:[ \t]+(?P<operand_1>{_OPERAND})(?:[ \t]*,[ \t]*(?P<operand_2>{_OPERAND})(?:[ \t]*,[ \t]*(?P<operand_3>{_OPERAND}))?)?)?)"
